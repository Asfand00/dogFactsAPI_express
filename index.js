...
// Define the GET /facts endpoint
app.get('/facts', (req, res) => {
    // Parse the 'number' query parameter into an integer
    const number = parseInt(req.query.number);
    
    // Initialize the response object
    let response = {};

    // Switch-case statement for handling different cases
    switch (true) {
        case isNaN(number): // Invalid number parameter
            response = {
                success: false,
                message: "Invalid number parameter. Please provide a valid integer."
            };
            break;
        case (number < 0 || number > dogFacts.length): // Out of range number parameter
            response = {
                success: false,
                message: `Invalid number parameter. Please provide a number between 1 and ${dogFacts.length}.`
            };
            break;
        default: // Valid number parameter
